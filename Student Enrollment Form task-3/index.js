...
const setData = (nam, gend, emai, we, sk, im) => {
    let temp = `<tr class="tabRow"><td><div class="desc"><p id="Sname">${nam}</p><p id="Sgender">${gend}</p><p id="Semail">${emai}</p><p id="Sweb"><a href=${"https://" + we} target="_blank" >${we}</a></p><p id="Sskils">${sk}</p> </div></td><td><div><img src=${im} alt="" onerror="this.onerror=null;this.src='${gend == "Male" ? './img/profileboy.svg' : './img/profilegirl.svg'}';" class="profImg"></div></td></tr>`
...
submit.addEventListener("click", (e) => {
    ...
    if (valid == false) {
        ...
        alert("Name is mandatory to Show Your information.Please enter your full name ");
